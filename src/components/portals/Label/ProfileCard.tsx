...
export const ProfileCard: React.FC<PublicProfileModalProps> = ({
  ...
  const [storefrontLabelData, setStorefrontLabelData] = React.useState<any>(null);
                  ...
                  >
                    ...
                    >
                      {selectedUserProfile.avatar && (selectedUserProfile.avatar.startsWith('http') || selectedUserProfile.avatar.startsWith('data:image') || selectedUserProfile.avatar.startsWith('/')) ? (
                        ...
                        <span className="font-bold">{selectedUserProfile.avatar || (effTarget.name || 'L').slice(0, 2).toUpperCase()}</span>
                      )}
                        ...
                        {showStorefront && (
                          <button
                            ...
                            onClick={(e) => {
                              ...
                              setStorefrontLabelData(targetData);
                    ...
                    selectedUserProfile.musicCatalog && selectedUserProfile.musicCatalog.length > 0 ? (
                      <div className="space-y-6">
                        {selectedUserProfile.musicCatalog.map((release: any, rIdx: number) => {
                          ...
                          const relUniqueId = String(release.id || `lbl-rel-${rIdx}-${(release.title || 'album').replace(/[^a-zA-Z0-9]/g, '_')}`);
    ...
    {isIsolatedStorefrontOpen && (
      <div className="fixed inset-0 z-[10000000] bg-black/95 backdrop-blur-md flex flex-col animate-fadeIn pointer-events-auto">
        <PublicStorefrontView
          labelName={storefrontLabelData?.name || selectedUserProfile?.name || 'Label Store'}
          activeBandId={storefrontLabelData?.id || selectedUserProfile?.id}
          activeBand={storefrontLabelData || selectedUserProfile}
