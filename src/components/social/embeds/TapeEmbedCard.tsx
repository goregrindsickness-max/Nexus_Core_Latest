...
export const TapeEmbedCard: React.FC<TapeEmbedCardProps> = ({
  ...
  // -------------------------------------------------------------
  // VARIANT 1: PUBLIC PROFILE CARD (Compact, 1.58 ratio, fluid SVG)
  // -------------------------------------------------------------
  if (isProfile) {
    return (
  ...
  // -------------------------------------------------------------
  // VARIANT 2: FULL SOCIAL FEED (Original layout & aspect-[1.46])
  // -------------------------------------------------------------
  return (
    <div className="bg-[#050505] rounded-xl p-4 my-3 border border-zinc-900 shadow-inner overflow-hidden relative group select-none">
      ...
      <div className="flex flex-col gap-4 relative z-10">
        {/* Cassette Tape Body (Feed aspect-[1.46]) */}
        <div className="relative w-full aspect-[1.46] bg-[#1a1a1a] rounded-lg shadow-2xl border border-[#0a0a0a] overflow-hidden flex flex-col p-2 ring-1 ring-white/10">
          {/* Texture overlay for the plastic shell */}
          <div className="absolute inset-0 opacity-40 pointer-events-none" style={{ backgroundImage: 'radial-gradient(#444 1px, transparent 1px)', backgroundSize: '3px 3px' }}></div>

          {/* 4 Corner Screws */}
          <div className="absolute top-1.5 left-1.5 w-3 h-3 rounded-full bg-black flex items-center justify-center shadow-[inset_0_1px_1px_rgba(255,255,255,0.2)] z-20">
            <div className="w-2 h-0.5 bg-zinc-600 rotate-45"></div>
          </div>
          <div className="absolute top-1.5 right-1.5 w-3 h-3 rounded-full bg-black flex items-center justify-center shadow-[inset_0_1px_1px_rgba(255,255,255,0.2)] z-20">
            <div className="w-2 h-0.5 bg-zinc-600 -rotate-45"></div>
          </div>
          <div className="absolute bottom-1.5 left-1.5 w-3 h-3 rounded-full bg-black flex items-center justify-center shadow-[inset_0_1px_1px_rgba(255,255,255,0.2)] z-20">
            <div className="w-2 h-0.5 bg-zinc-600 rotate-90"></div>
          </div>
          <div className="absolute bottom-1.5 right-1.5 w-3 h-3 rounded-full bg-black flex items-center justify-center shadow-[inset_0_1px_1px_rgba(255,255,255,0.2)] z-20">
            <div className="w-2 h-0.5 bg-zinc-600"></div>
          </div>

          {/* Main Label / Sticker Area */}
          <div className="relative z-10 mx-2 mt-1 flex-1 bg-[#181818] rounded-sm shadow-[0_0_2px_rgba(0,0,0,1)] border border-white/5 flex flex-col items-center justify-between p-1.5 pb-1">
            ...
            <div className="w-full px-1 z-20">
              <div className="w-full bg-[#09090b] border border-zinc-800/80 px-2 py-1 flex flex-col gap-0.5 shadow-sm rounded-xs">
                {/* Line 1: Band / Artist Name */}
                <div className="flex items-center justify-between gap-2 overflow-hidden border-b border-zinc-800/60 pb-0.5">
                  <div className="flex items-center gap-1.5 min-w-0 flex-1 overflow-hidden">
                    <span className="w-1.5 h-1.5 rounded-full bg-amber-400 shrink-0 shadow-[0_0_4px_rgba(251,191,36,0.6)]" />
                    <span className="text-[10px] font-mono font-bold text-zinc-100 uppercase truncate tracking-wider">{bandName}</span>
                  </div>
                  {tapeDate && (
                    <span className="text-[8px] font-mono font-medium text-zinc-400 uppercase shrink-0 px-1 py-0.2 bg-zinc-900/90 rounded border border-zinc-800/80">{tapeDate}</span>
                  )}
                </div>
                ...
                <div className="flex items-center gap-1.5 min-w-0 overflow-hidden pt-0.5">
                  <span className="w-1.5 h-1.5 rounded-full bg-rose-500 shrink-0 shadow-[0_0_4px_rgba(244,63,94,0.6)]" />
                  <span className="text-[10px] font-mono font-bold text-rose-400 uppercase truncate tracking-wider">{tapeTitle}</span>
                </div>
              </div>
            </div>

            {/* Clear Acrylic Window Area with Spools (Feed full height) */}
            <div className="w-[88%] h-[60px] sm:h-[68px] bg-[#09090b] rounded-xs flex items-center justify-between px-3 sm:px-4 relative overflow-hidden border border-zinc-700/60 shadow-[inset_0_0_12px_rgba(0,0,0,0.95)] my-1">

              {/* Tape gauge ruler markings in center window */}
              <div className="absolute left-1/2 -translate-x-1/2 flex flex-col items-center justify-center gap-0.5 h-[80%] z-10 opacity-80 pointer-events-none">
                <span className="text-[7px] font-mono font-bold text-zinc-400 leading-none">100</span>
                <div className="flex flex-col items-center gap-[2px] my-0.5">
                  <div className="w-5 sm:w-6 h-[1.5px] bg-zinc-500 rounded-full"></div>
                  <div className="w-3 sm:w-4 h-[1px] bg-zinc-600"></div>
                  <div className="w-5 sm:w-6 h-[1.5px] bg-zinc-500 rounded-full"></div>
                  <div className="w-3 sm:w-4 h-[1px] bg-zinc-600"></div>
                  <div className="w-5 sm:w-6 h-[1.5px] bg-zinc-500 rounded-full"></div>
                </div>
                <span className="text-[7px] font-mono font-bold text-zinc-400 leading-none">0</span>
              </div>

              {/* Left Reel */}
              <div className="w-12 h-12 sm:w-14 sm:h-14 rounded-full bg-[#141414] flex items-center justify-center relative shrink-0 z-0 border border-zinc-800 shadow-md">
                <div 
                  className="absolute rounded-full bg-gradient-to-br from-[#4d2c19] via-[#3d2011] to-[#1c0e07] border border-[#5a321c]" 
                  style={{ 
                    inset: `${2 + (progress / 100) * 8}px`, 
                    borderWidth: `${8 - (progress / 100) * 8}px` 
                  }}
                />
                <div className="absolute inset-0 rounded-full border border-zinc-700/30 pointer-events-none" />
                <div className={`w-8 h-8 sm:w-9 sm:h-9 bg-[#f4f4f5] rounded-full flex items-center justify-center z-10 shadow-[0_1px_3px_rgba(0,0,0,0.8)] ${isPlaying ? 'animate-spin' : ''}`} style={{ animationDuration: '2.5s', animationTimingFunction: 'linear' }}>
                  <div className="w-1.5 h-1.5 bg-[#141414] absolute top-0.5 rounded-xs" />
                  <div className="w-1.5 h-1.5 bg-[#141414] absolute bottom-0.5 rounded-xs" />
                  <div className="w-1.5 h-1.5 bg-[#141414] absolute left-0.5 rounded-xs" />
                  <div className="w-1.5 h-1.5 bg-[#141414] absolute right-0.5 rounded-xs" />
              ...
              <div className="w-12 h-12 sm:w-14 sm:h-14 rounded-full bg-[#141414] flex items-center justify-center relative shrink-0 z-0 border border-zinc-800 shadow-md">
                <div 
                  ...
                  style={{ 
                    ...
                    borderWidth: `${2 + (progress / 100) * 8}px` 
                  }}
                />
                <div className="absolute inset-0 rounded-full border border-zinc-700/30 pointer-events-none" />
                <div className={`w-8 h-8 sm:w-9 sm:h-9 bg-[#f4f4f5] rounded-full flex items-center justify-center z-10 shadow-[0_1px_3px_rgba(0,0,0,0.8)] ${isPlaying ? 'animate-spin' : ''}`} style={{ animationDuration: '2.5s', animationTimingFunction: 'linear' }}>
            ...
            {/* Label Text Bottom Footer */}
            <div className="w-full px-2 flex justify-between items-center z-20">
              <span className="text-[8px] font-bold font-mono text-zinc-500 uppercase tracking-widest">NEXUS CHROMIUM HIGH-BIAS</span>
              <span className="text-[8px] font-bold font-mono text-zinc-600 uppercase">90 MIN</span>
            </div>
          </div>

          {/* Bottom Tape Head Trapezoid Guard (Original Feed Style) */}
          <div className="relative z-10 mx-auto w-full px-2 pt-1 flex justify-center">
            <svg 
              className="w-full max-w-[340px] h-auto text-zinc-300 block" 
              viewBox="0 0 240 38" 
              preserveAspectRatio="xMidYMid meet" 
              fill="none"
            >
        ...
        <div className="flex flex-col gap-1.5 px-2">
          <div 
            className="h-2.5 w-full bg-zinc-900 rounded-full border border-zinc-800 overflow-hidden relative shadow-inner cursor-pointer"
            onClick={(e) => {
              const rect = e.currentTarget.getBoundingClientRect();
              const percent = ((e.clientX - rect.left) / rect.width) * 100;
              onSeek(Math.max(0, Math.min(100, percent)));
        ...
        {/* Controls Area (Full Feed Bar) */}
        <div className="flex items-center justify-between gap-2 px-2 pt-1">
          <div className="text-[9px] font-mono text-zinc-500 uppercase font-bold tracking-wider border border-zinc-900 px-2 py-1 rounded bg-zinc-900/40">
            DATE: {tapeDate || 'UNKNOWN'}
          </div>
          ...
          <div className="flex items-center gap-1.5 sm:gap-2">
            <button 
              type="button"
              onClick={() => onSeek(Math.max(0, progress - 10))}
              className="w-8 h-8 rounded bg-zinc-900/80 border border-zinc-800 hover:border-zinc-700 flex items-center justify-center text-zinc-400 hover:text-zinc-200 transition-colors cursor-pointer active:scale-95 shadow-sm"
              title="Skip back 10%"
            >
              <svg className="w-3.5 h-3.5 fill-current" viewBox="0 0 24 24"><path d="M11 18V6l-8.5 6 8.5 6zm.5-6l8.5 6V6l-8.5 6z"/></svg>
            </button>
            ...
            <button 
              ...
              className="w-10 h-10 rounded-full bg-zinc-900 border border-red-900/60 hover:border-red-500 flex items-center justify-center text-white transition-all group relative shadow-[0_0_15px_rgba(239,68,68,0.2)] mx-1 cursor-pointer active:scale-95"
              title={isPlaying ? 'Pause tape' : 'Play tape'}
            >
              {!isPlaying ? (
                <svg className="w-4 h-4 fill-current ml-0.5 group-hover:text-red-400 transition-colors" viewBox="0 0 24 24"><path d="M8 5v14l11-7z"/></svg>
              ) : (
                <svg className="w-4 h-4 fill-current group-hover:text-red-400 transition-colors" viewBox="0 0 24 24"><path d="M6 19h4V5H6v14zm8-14v14h4V5h-4z"/></svg>
              )}
              {isPlaying && <div className="absolute inset-0 rounded-full border border-red-500/30 scale-110 animate-ping opacity-25"></div>}
            </button>
            ...
            <button 
              ...
              title="Skip forward 10%"
            >
              <svg className="w-3.5 h-3.5 fill-current" viewBox="0 0 24 24"><path d="M4 18l8.5-6L4 6v12zm9-12v12l8.5-6L13 6z"/></svg>
            </button>
          </div>

          <div className={`flex items-center gap-1.5 text-[9px] font-mono font-bold px-2.5 py-1 rounded transition-colors ${isPlaying ? 'text-red-500 border border-red-500/30 bg-red-950/20 shadow-[0_0_8px_rgba(239,68,68,0.2)]' : 'text-red-900 border border-red-950/30 bg-red-950/10'}`}>
            <div className={`w-1.5 h-1.5 rounded-full ${isPlaying ? 'bg-red-500 animate-pulse' : 'bg-red-900'}`}></div>
            {isPlaying ? 'PLAYING' : 'READY'}
          </div>
        </div>
