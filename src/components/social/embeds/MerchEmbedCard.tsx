...
import { Flame, Zap, ZoomIn, Handshake, X, ShoppingCart, Timer, ShieldAlert, Sparkles, ShoppingBag, Package, Check, ChevronLeft, ChevronRight } from 'lucide-react';
...
export const MerchEmbedCard: React.FC<MerchEmbedCardProps> = ({
  ...
  // Drop & Stock calculations
  const isTimedDrop = Boolean(post.merchData.isTimed && (post.merchData.expiresAt || post.merchData.durationHours));
  const isUnlimited = Boolean(post.merchData.isUnlimited || (post.merchData.stock === undefined && post.merchData.totalStock === undefined));
  const totalLimit = post.merchData.totalStock || post.merchData.stock || 50;
  const currentStock = isUnlimited ? null : (post.merchData.stock ?? totalLimit);
  const claimedPercent = isUnlimited ? 0 : Math.min(100, Math.max(0, Math.round(((totalLimit - (currentStock ?? totalLimit)) / totalLimit) * 100)));
  const isCriticalStock = !isUnlimited && currentStock !== null && currentStock <= 5;

  // Multi-image list (strict max 3)
  const allImages: string[] = React.useMemo(() => {
    const list = post.merchData?.images && post.merchData.images.length > 0
      ? post.merchData.images
      : (post.merchData?.thumbnail ? [post.merchData.thumbnail] : []);
    return list.filter(Boolean).slice(0, 3);
  }, [post.merchData]);

  const [activeImageIdx, setActiveImageIdx] = useState(0);
  const currentIdx = Math.min(activeImageIdx, Math.max(0, allImages.length - 1));
  const currentDisplayImage = allImages[currentIdx] || post.merchData.thumbnail;

  return (
    <div className={`bg-[#0f0d11] rounded-2xl overflow-hidden mb-3.5 transition-all ${
      isTimedDrop
        ? 'border border-orange-500/50 shadow-[0_0_25px_rgba(249,115,22,0.15)]'
        : 'border border-zinc-800 shadow-lg'
    }`}>
      {/* Top Banner: Drop Type & Inventory Urgency Bar */}
      {isTimedDrop ? (
        <div className={`px-3 py-1.5 border-b flex items-center justify-between gap-2 text-xs font-mono font-black uppercase ${
          ...
          <div className="flex items-center gap-1.5">
            ...
            <span>{isCriticalStock ? '⚡ CRITICAL LOW STOCK' : '⚡ LIMITED FLASH DROP'}</span>
          </div>
          <div className="flex items-center gap-2">
            ...
            <span className={`px-2 py-0.5 rounded text-[10px] font-extrabold ${
              ...
              {isUnlimited ? 'FLASH WINDOW ACTIVE' : `${currentStock} / ${totalLimit} LEFT`}
            </span>
          </div>
        </div>
      ) : (
        <div className="px-3 py-1.5 border-b border-zinc-800/80 bg-zinc-950/90 flex items-center justify-between gap-2 text-xs font-mono font-black uppercase text-zinc-300">
          <div className="flex items-center gap-1.5">
            {isUnlimited ? (
              <>
                <ShoppingBag className="w-3.5 h-3.5 text-emerald-400" />
                <span className="text-zinc-200">OFFICIAL STORE DROP</span>
              </>
            ) : isCriticalStock ? (
              <>
                <ShieldAlert className="w-3.5 h-3.5 text-red-400 animate-pulse" />
                <span className="text-red-400">LOW STOCK WARNING</span>
              </>
            ) : (
              <>
                <Package className="w-3.5 h-3.5 text-amber-400" />
                <span className="text-zinc-200">OFFICIAL MERCH DROP</span>
              </>
            )}
          </div>
          <div>
            {isUnlimited ? (
              ...
              <span className="px-2 py-0.5 rounded text-[10px] font-extrabold bg-red-500 text-black">
                ONLY {currentStock} LEFT
              </span>
            ) : (
              <span className="px-2 py-0.5 rounded text-[10px] font-bold bg-zinc-900 text-zinc-300 border border-zinc-700/80">
                {currentStock} / {totalLimit} AVAILABLE
              </span>
            )}
          </div>
        </div>
      ...
      {/* Progress Bar for Stock Depletion (Only when limited stock is active) */}
      ...
      <div className="flex flex-col sm:flex-row">
        ...
        <div 
          onClick={() => openMerchLightbox(post, currentIdx)}
        ...
        >
          ...
          {allImages.length > 1 && (
            <div className="absolute top-2.5 right-2.5 bg-black/85 backdrop-blur-md text-zinc-300 border border-zinc-700/80 text-[9px] font-mono font-bold px-1.5 py-0.5 rounded shadow z-10 flex items-center gap-1 pointer-events-none">
          ...
          {allImages.length > 1 && (
            <>
              <button
                ...
                className="absolute left-1.5 top-1/2 -translate-y-1/2 w-6 h-6 rounded-full bg-black/80 hover:bg-orange-500 text-white hover:text-black flex items-center justify-center transition-all opacity-0 group-hover/merch-img:opacity-100 z-10 shadow-md cursor-pointer border border-zinc-700/60"
              ...
              <button
                ...
                className="absolute right-1.5 top-1/2 -translate-y-1/2 w-6 h-6 rounded-full bg-black/80 hover:bg-orange-500 text-white hover:text-black flex items-center justify-center transition-all opacity-0 group-hover/merch-img:opacity-100 z-10 shadow-md cursor-pointer border border-zinc-700/60"
                title="Next image"
              >
                <ChevronRight className="w-3.5 h-3.5" />
              </button>

              {/* Bottom Miniature Thumbnails */}
              <div 
                className="absolute bottom-2.5 left-2.5 z-10 flex items-center gap-1 bg-black/80 backdrop-blur-md p-0.5 rounded-lg border border-zinc-800/90 shadow-md"
              ...
              >
                {allImages.map((img, i) => (
                  <button
                    ...
                    className={`w-5 h-5 rounded overflow-hidden border transition-all cursor-pointer ${
                      currentIdx === i
          ...
          <div className="absolute bottom-2.5 right-2.5 bg-black/80 backdrop-blur-md text-orange-400 border border-orange-500/40 text-[9px] font-mono font-bold px-2 py-1 rounded-md flex items-center gap-1 shadow-lg opacity-90 group-hover/merch-img:opacity-100 transition-all z-10">
        ...
        <div className="w-full sm:w-3/5 p-3.5 sm:p-4.5 flex flex-col justify-between space-y-2">
          <div>
            ...
            <h4 className="text-white font-mono font-bold text-sm sm:text-base leading-snug mb-1">
              {post.merchData.name}
            </h4>

            {post.merchData.description && (
              <p className="text-xs text-zinc-400 font-sans leading-relaxed mb-2 line-clamp-2">
                {post.merchData.description}
              </p>
            )}
            ...
            <div className="flex flex-wrap items-center justify-between gap-2 mb-2">
              ...
              {/* Active Countdown Timer for Timed Exclusive Releases ONLY */}
              {Boolean(post.merchData.isTimed && (post.merchData.expiresAt || post.merchData.durationHours)) && (
