...
export const getRoleBorderAndGlowClass = (role: string = '', userProfile?: any): string => {
  const r = (role || '').toLowerCase();
  const ws = (userProfile?.active_workspace || '').toLowerCase();
  const acc = (userProfile?.account_type || '').toLowerCase();

  if (r === 'band' || r.includes('band') || r.includes('artist') || ws === 'band') {
    return 'border-2 border-[#39ff14] shadow-[0_0_15px_rgba(57,255,20,0.8)]';
  }
  if (r === 'promoter' || ws === 'promoter' || r.includes('promoter') || r.includes('venue')) {
    return 'border-2 border-yellow-400 shadow-[0_0_15px_rgba(234,179,8,0.8)]';
  }
  if (r === 'label' || r.includes('label') || ws === 'label') {
    return 'border-2 border-[#ff6c00] shadow-[0_0_15px_rgba(255,108,0,0.8)]';
  }
  if (r === 'creative' || r.includes('creative') || ws === 'creative') {
    return 'border-2 border-[#ff00ff] shadow-[0_0_15px_rgba(255,0,255,0.8)]';
  }
  if (r === 'fan_only' || r.includes('fan')) {
    return 'border-2 border-[#00d2ff] shadow-[0_0_15px_rgba(0,210,255,0.8)]';
  }
  if (r === 'industry_pro' || r === 'industry pro' || r.includes('industry') || acc === 'industry_pro' || acc === 'industry pro' || acc === 'pro') {
